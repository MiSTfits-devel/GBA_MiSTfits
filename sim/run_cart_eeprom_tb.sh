...
python3 sim/tests/gen_eeprom_probe.py
python3 - sim/tests/eeprom_probe.gba sim/tests/eeprom_words.hex <<'PYEOF'
import struct, sys
...
nvc -L "$LIBS" --work=work:"$LIBS"/WORK -a --relaxed \
   ...
   rtl/gba_top.vhd \
   sim/tb_cart_eeprom.vhd

nvc -L "$LIBS" --work=work:"$LIBS"/WORK -e tb_cart_eeprom

nvc -L "$LIBS" --work=work:"$LIBS"/WORK -r tb_cart_eeprom \
